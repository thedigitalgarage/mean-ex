...
In this MEAN stack bootstrap you will have [Socrata library](https://www.npmjs.com/package/node-socrata) ready to use just providing your socrata app token, you can configure that token in the code or through an environment variable ``SOCRATA_APP_TOKEN``. After that you just need to add ``/socrata-example`` to the URL where you see the "Hello World" message and a JSON object will appear to you, which is information being provided by Socrata.
